...
function EvaluatingDetail({ app, onBack, onPass, onFail, showToast }) {
  ...
  const ScoreRow = ({ qKey, label, auto }) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid #f5f5f5" }}>
      <div>
        ...
        <span style={{ fontSize: 9, fontWeight: 200, color: "#ccc", marginLeft: 6 }}>{auto ? "auto" : "0–100"}</span>
      </div>
      {auto != null
        ? <ScoreChip score={auto} />
        : <input className="score-input" type="number" min="0" max="100"
            value={manualScores[qKey]}
            onChange={e => setManualScores(p => ({ ...p, [qKey]: e.target.value }))}
            style={{ width: 48, border: "none", borderBottom: "1px solid #e8e8e4", fontFamily: sans, fontSize: 13, fontWeight: 300, color: "#1a1a1a", background: "transparent", textAlign: "right", padding: "2px 0" }}
          />
      }
    </div>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 268px", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ position: "sticky", top: 36, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #f0f0f0", padding: "18px 22px" }}>
            ...
            <ScoreRow qKey="q1" label="Q1 visual instinct" auto={autoQ1} />
            <ScoreRow qKey="q2" label="Q2 empathy" />
            <ScoreRow qKey="q3" label="Q3 design ranking" auto={autoQ3} />
            <ScoreRow qKey="q4" label="Q4 local brands" />
            <ScoreRow qKey="q5" label="Q5 intl brands" />
            <ScoreRow qKey="q6" label="Q6 dimension" />
            <ScoreRow qKey="q7" label="Q7 moodboard" />
